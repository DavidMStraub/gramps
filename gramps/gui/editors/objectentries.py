    ...
    def _update_dnd_capability(self):
        ...
        if not self.label.drag_dest_get_target_list():
            self.label.drag_dest_set(
                Gtk.DestDefaults.ALL, [self._DND_TYPE.target()], Gdk.DragAction.COPY
            )
...
class PersonEntry(ObjEntry):
    ...
    def call_editor(self, obj=None):
        ...
        try:
            EditPerson(self.dbstate, self.uistate, self.track, person, func)
...
class PlaceEntry(ObjEntry):
    ...
    EMPTY_TEXT = "<i>%s</i>" % _(
        "To select a place, use drag-and-drop " "or use the buttons"
    )
    ...
    def call_editor(self, obj=None):
        if obj is None:
            ...
            func = self.after_edit
        try:
            EditPlace(self.dbstate, self.uistate, self.track, place, func)
        except WindowActiveError:
            pass
...
class SourceEntry(ObjEntry):
    ...
    EMPTY_TEXT = "<i>%s</i>" % _(
        "To select a source, use drag-and-drop " "or use the buttons"
    )
...
class MediaEntry(ObjEntry):
    ...
    EMPTY_TEXT = "<i>%s</i>" % _(
        "To select a media object, use drag-and-drop " "or use the buttons"
    )
...
class NoteEntry(ObjEntry):
    ...
    EMPTY_TEXT = "<i>%s</i>" % _(
        "To select a note, use drag-and-drop " "or use the buttons"
    )
