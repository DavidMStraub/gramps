...
class NameFormat(GenericFormat):
    ...
    def parse_format(self, name):
        ...
        def common():
            ...
            return name.get_call_name() or name.get_first_name().split(" ")[0]
...
class DateFormat(GenericFormat):
    ...
    def parse_format(self, date):
        ...
        def eu_month():
            return month(date.get_stop_month(), self.__count_chars("N", 4)).upper()
...
class EventFormat(GenericFormat):
    ...
    def parse_format(self, event):
        ...
        def format_attrib():
            ...
            if name:
                return attrib_parse.get_attribute(event.get_attribute_list(), name)
            else:
                return
    ...
    def parse_empty(self):
        ...
        code = "dDa"
        function = [self.__empty_format, self.__empty_format, self.__empty_attrib]
...
class GalleryFormat(GenericFormat):
    ...
    def parse_format(self, photo):
        ...
        def format_attrib():
            ...
            name = attrib_parse.get_name()
            if name:
                return attrib_parse.get_attribute(photo.get_attribute_list(), name)
            else:
                return
...
class VariableParse:
    ...
    def is_a(self):
        ...
        return (
            self._in.this == "$"
            and self._in.next is not None
            and "nsijbBdDmMvVauetTpPG".find(self._in.next) != -1
        )
    ...
    def parse_format(self):
        ...
        if next_char == "n":
            ...
            if self.empty_item(self.friend.family):
                return
            return self.__parse_date(
                self.get_event_by_type(self.friend.family, EventType.MARRIAGE)
            )
        elif next_char == "v":
            # Divorce date
            if self.empty_item(self.friend.family):
                return
            return self.__parse_date(
                self.get_event_by_type(self.friend.family, EventType.DIVORCE)
            )
        elif next_char == "T":
            # Todays date
            ...
            if self.empty_attribute(self.friend.person):
                return
            return attrib_parse.parse_format(self.friend.person.get_attribute_list())
        elif next_char == "u":
            # Marriage Atribute
            if self.empty_attribute(self.friend.family):
                return
            return attrib_parse.parse_format(self.friend.family.get_attribute_list())
...
class SubstKeywords:
    ...
    def __init__(
        self, database, locale, name_displayer, person_handle, family_handle=None
    ):
    ...
    def __parse_line(self):
        ...
        variable = VariableParse(self, self.database, self.line, self._locale, self._nd)
...
if __name__ == "__main__":
    ...
    def date_set():
        date_to_test.set_yr_mon_day(
            1970 if 0 in y_or_n else 0, 9 if 1 in y_or_n else 0, 3 if 2 in y_or_n else 0
        )
        # print date_to_test
    ...
    for y_or_n in combinations(3, 1):
        ...
        print(tmp)
        answer.append(tmp)
    print("Good" if answer == ["1970 d1970f70", " a99b09", " c3"] else "!! bad !!")
    ...
    print(
        "Good"
        if answer == ["1970 a99b09d1970f70", "1970 c3d1970f70", " a99b09c3"]
        else "!! bad !!"
    )
    ...
    print(tmp)
    answer.append(tmp)
    print("Good" if answer == ["1970 a99b09c3d1970f70"] else "!! bad !!")
    ...
    print(answer)
    print("Good" if answer == "BobDr.2Billy3Buck4BobIV6The Clubs" else "!! bad !!")
    ...
    def place_set():
        ...
        main_loc.set_county("Butte" if 3 in y_or_n else "")
        main_loc.set_state("Idaho" if 4 in y_or_n else "")
        main_loc.set_postal_code("83213" if 5 in y_or_n else "")
        main_loc.set_country("USA" if 6 in y_or_n else "")
        main_loc.set_parish("St Anns" if 7 in y_or_n else "")
        place_to_test.set_title("Atomic City" if 8 in y_or_n else "")
        place_to_test.set_longitude("N43H38'5\"N" if 9 in y_or_n else "")
        place_to_test.set_latitude("W113H18'5\"W" if 10 in y_or_n else "")

    # code = "txy"
    line_in = (
        "$(e)<1>{<2>$(l) <3> $(c)<4><0><5>{$(s)<6>$(p)<7>"
