...
class OdfBackend(DocBackend):
    ...
    STYLETAG_MARKUP = {
        DocBackend.BOLD: ('<text:span text:style-name="Tbold">', "</text:span>"),
        DocBackend.ITALIC: ('<text:span text:style-name="Titalic">', "</text:span>"),
        DocBackend.UNDERLINE: (
    ...
    def _create_xmltag(self, tagtype, value):
        ...
        if tagtype == DocBackend.FONTCOLOR:
            return (
                '<text:span text:style-name="FontColor__%s__">'
                % value.replace("#", ""),
                "</text:span>",
            )
        elif tagtype == DocBackend.FONTFACE:
            return (
                '<text:span text:style-name="FontFace__%s__">'
                % self.ESCAPE_FUNC()(value).replace(" ", "-"),
                "</text:span>",
            )
        elif tagtype == DocBackend.FONTSIZE:
            return (
                '<text:span text:style-name="FontSize__%d__">' % value,
                "</text:span>",
            )
        else:  # elif ( tagtype == DocBackend.HIGHLIGHT ):
            return (
                '<text:span text:style-name="FontHighlight__%s__">'
                % value.replace("#", ""),
                "</text:span>",
            )
    ...
    def format_link(self, value):
        ...
        if value.startswith("gramps://"):
            ...
            return ('<text:a xlink:href="%s">' % self.ESCAPE_FUNC()(value), "</text:a>")
